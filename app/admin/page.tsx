...
export default function AdminPage() {
  ...
  type StorageData = Penduduk[] | Berita[] | Kegiatan[];
  const saveToStorage = (key: string, data: StorageData) => {
  ...
  const updatePenduduk = () => {
    ...
    const updated = penduduk.map((p) =>
      p.id === editingId ? ({ ...p, ...formPenduduk } as Penduduk) : p
    );
    setPenduduk(updated);
    saveToStorage("desa-penduduk", updated);
    setFormPenduduk({});
  ...
  const updateBerita = () => {
    ...
    const updated = berita.map((b) =>
      b.id === editingId ? ({ ...b, ...formBerita } as Berita) : b
    );
    setBerita(updated);
    saveToStorage("desa-berita", updated);
    setFormBerita({});
  ...
  const updateKegiatan = () => {
    ...
    const updated = kegiatan.map((k) =>
      k.id === editingId ? ({ ...k, ...formKegiatan } as Kegiatan) : k
    );
    setKegiatan(updated);
    saveToStorage("desa-kegiatan", updated);
    setFormKegiatan({});
